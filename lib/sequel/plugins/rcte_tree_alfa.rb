    ...
    module RcteTreeAlfa
      ...
      def self.apply(model, opts=OPTS)
        ...
        a[:eager_loader] ||= proc do |eo|
          ...
          table_alias = model.dataset.schema_and_table(model.table_name)[1].to_sym
          if model.db.adapter_scheme == :mysql || model.db.adapter_scheme == :mysql2
            current_pks = children_map.keys
            while current_pks.any?
              elds = model.eager_loading_dataset(r, model.where(prkey=>current_pks.dup), r.select, eo[:associations], eo)
              current_pks.clear
              elds.all do |obj|
                opk = prkey_conv[obj]
                obj.associations[parent] = nil
